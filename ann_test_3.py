...
ann = NeuralNetwork([2, 2, 3])

for i in range(trainings):
    num1 = random.randint(-15, 4)
    num2 = random.randint(num1 + 1, num1 + 11)
    ann.feedForward([num1, num2])

    ann.calcCosts([1, 0, 0])
    ann.backProp([1, 0, 0], 0.1)

    num = random.randint(-15, 15)
    ann.feedForward([num, num])

    ann.calcCosts([0, 1, 0])
    ann.backProp([0, 1, 0], 0.1)

    num1 = random.randint(-4, 15)
    num2 = random.randint(num1 - 11, num1 - 1)
    ann.feedForward([num1, num2])

    ann.calcCosts([0, 0, 1])
    ann.backProp([0, 0, 1], 0.1)
...
for i in range(tests):
    ...
    temp = [int(num1 < num2), int(num1 == num2), int(num1 > num2)][np.argmax(np.array(ann.activations[-1]))]
    if temp:
        correct += 1
    else:
        print(num1, num2, ann.activations[-1])
